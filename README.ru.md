...
Для обычного обновления этого достаточно - сотрудники, курсы и результаты тестов живут в
именованных Docker-томах, которые пересборка образов не трогает, а контейнер `backend` сам
накатывает накопившиеся миграции базы при старте. Единственное, что нельзя запускать на боевом
окружении - `docker compose down -v`: флаг `-v` безвозвратно удаляет эти тома вместе с базой.
Перед обновлением сверьтесь с [CHANGELOG.ru.md](CHANGELOG.ru.md) на предмет версии, на которую
обновляетесь, а в [docs/upgrading.ru.md](docs/upgrading.ru.md) - как сделать бэкап базы перед
обновлением, откатиться назад и разворачиваться с конкретного тега версии вместо плавающего
`main`.
